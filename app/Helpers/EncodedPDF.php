...
class EncodedPDF extends Rule
{
	...
	protected function createTemporaryFile($data)
	{
		...
		return new UploadedFile(
			stream_get_meta_data($this->file)['uri'], 'application',
    ...
    {
		if (! Str::startsWith($value, "data:application/{$this->parameters[0]};base64,")) {
			return false;
		}

		$result = validator(['file' => $this->createTemporaryFile($value)], ['file' => 'file'])
			   -> passes();
    ...
    public function message()
    {
        return Helper::getLocalizedErrorMessage(
            'encoded_file',
            "El :attribute no tiene el formato {$this->parameters[0]} requerido"
        );
    }
