	...
	<div class="container">
		<div class="block">
			<div class="block-header bg-primary-dark">
				<h3 class="block-title text-white text-center">AUTORIDADES</h3>
    			<button type="button" data-toggle="tooltip" title="Nueva Autoridad" @click="showModal('AuthorityModal',null,'Nueva Autoridad','store')" class="btn btn-success">
			    	...
			    	<table class="table table-hover table-vcenter">
			            ...
			            <tbody>
			                ...
			                <tr v-else v-for="(item_table,index_for_table) in table_data" :key="index_for_table">
                                ...
                                <td class="font-w600 font-size-sm" style="text-transform: capitalize;" v-text="item_table.type"></td>
			                    <td class="text-center">
			                        <div class="btn-group">
			                            <button type="button" @click="showModal('AuthorityModal',item_table,'Editar datos de Autoridad','edit')" class="btn btn-sm btn-light" data-toggle="tooltip" title="Editar datos Autoridad">
			                                <i class="fa fa-fw fa-pencil-alt"></i>
			                            </button>
			                            <button type="button" @click="deleteData(item_table.id)" class="btn btn-sm btn-light" data-toggle="tooltip" title="Eliminar Autoridad">
        ...
        <div class="modal fade" id="AuthorityModal" tabindex="-1" role="dialog" aria-labelledby="AuthorityModal" aria-hidden="true">
                                ...
                                <div class="col-6">
                                	<div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" @keyup.enter="checkDocument" v-model="authorityData.person.nro_document" class="form-control">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6">
                                	<div class="form-group">
                                    	...
                                    	<v-select :disabled="!exist_document" label="name" v-model="authorityData.person.document" :options="list_documents"></v-select>
                                	</div>
                                </div>
                                <!-- col-12 -->
                                <div class="col-6">
                                	<div class="form-group">
                                		...
                                		<input :disabled="!exist_document" type="text" class="form-control" v-model="authorityData.person.firstname">
                                	</div>
                                </div>
                                <div class="col-6">
                                	<div class="form-group">
                                		<label for="">Apellidos</label>
                                		<input :disabled="!exist_document" type="text" class="form-control" v-model="authorityData.person.lastname">
                                	</div>
                                </div>
                                <!-- col-12 -->
                                <div class="col-4">
	                                <div class="form-group">
	                                    ...
	                                    <datepicker :disabled="!exist_document" :disabled-dates="no_dates" v-model="authorityData.person.birthday" input-class="bg-white form-control"></datepicker>
	                                </div>
                                </div>
                                <div class="col-8">
	                                <div class="form-group">
	                                    <label>Direccion</label>
	                                    <textarea :disabled="!exist_document" v-model="authorityData.person.direction" class="form-control"></textarea>
	                                </div>
                                </div>
                                <!-- col-12 -->
                                <div class="col-4">
	                                <div class="form-group">
	                                    ...
	                                    <input :disabled="!exist_document" v-model="authorityData.person.local_phone" class="form-control"></input>
	                                </div>
                                </div>
                                <div class="col-4">
	                                <div class="form-group">
	                                    <label>Telefono Movil</label>
	                                    <input :disabled="!exist_document" v-model="authorityData.person.movil_phone" class="form-control"></input>
	                                </div>
                                </div>
                                <div class="col-4">
	                                <div class="form-group">
	                                    <label>Correo Electronico</label>
	                                    <input :disabled="!exist_document" v-model="authorityData.person.mail_contact" class="form-control"></input>
	                                </div>
                                </div>
                                <!-- col-12 -->
                                <div class="col-12">
...
export default {
    ...
    data() {
		return {
            ...
            type_authorities:[
                'rector',
                'academic_vice_chancellor',
                'administrative_vice_chancellor',
                'other'
            ],
            authorityData:{
                id_authority:0,
	...
	methods:{
        ...
        checkDocument()
        {
        	if (this.authorityData.person.nro_document !== null) {
	        	this.document.nro 	= this.authorityData.person.nro_document
		...
		getData(page)
        {
            let url =  "/precarga/get-authorities" 
            axios.post(url,{
                page   : page, 
            	...
            	if (this.table_data.length > 0) {
                	this.$alertify.success('Exito al cargar Autoridades')
            	}else{
        ...
        storeData()
        {
            this.$root.loading('Verificando y guardando','Espere mientras se verifican los datos para registrar esta autoridad')
            let url = '/precarga/store-authority'
            axios.post(url,{
                authorityData :this.authorityData
            }).then(response => {
                swal.close()
                $("#AuthorityModal").modal('hide')
        		this.$alertify.success('La autoridad se registro con exito')
                this.getData()
            }).catch(errors => {
                swal.close()
                if (status = 204)
        ...
        updateData()
        {
        	this.$root.loading('Verificando y actualizando','Espere mientras se verifican los datos para actualizar esta autoridad')
            let url = '/precarga/update-authority'
            axios.post(url,{
                authorityData :this.authorityData
            }).then(response => {
                swal.close()
                $("#AuthorityModal").modal('hide')
        		this.$alertify.success('La autoridad se actualizo con exito')
                this.getData()
            }).catch(errors => {
                swal.close()
                if (status = 204)
        ...
        deleteData(idProf)
        {
        	swal({
                ...
                if (willDelete) {
        			this.$root.loading('Evaluando','Espere mientras se verifican los datos para eliminar esta autoridad')
		            let url = '/precarga/delete-authority'
		            axios.post(url,{
		                id:idProf
		            }).then(response => {
		                swal.close()
		                this.getData()
		        		this.$alertify.success('La autoridad fue eliminada con exito')
		            }).catch(errors => {
		                swal.close()
		            })
                }
